...
async function resolveTeamIdByCompetition(name, competitionId){
  ...
  let t = teams.find(x => norm(x.name) === target);
  if(!t){
    const parts = target.split(/\s+/).filter(Boolean);
    t = teams.find(x => parts.every(p => norm(x.name).includes(p)));
  }
...
export default function App(){
  ...
  async function loadFixtures(){
    const promises = COMPETITIONS.map(async c => {
    ...
    <div className="container">
      ...
      <div className="grid">
        ...
        <div className="card" style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div className="sub">Descargar históricos recientes y analizar</div>
