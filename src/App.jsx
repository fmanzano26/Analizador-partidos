...
function scoreMatrix(lh,la,max=10){const ph=Array.from({length:max+1},(_,i)=>pmf(i,lh));const pa=Array.from({length:max+1},(_,i)=>pmf(i,la));
  const M=Array.from({length:max+1},()=>Array(max+1).fill(0));let s=0;for(let i=0;i<=max;i++){for(let j=0;j<=max;j++){const v=ph[i]*pa[j];M[i][j]=v;s+=v;}}for(let i=0;i<=max;i++){for(let j=0;j<=max;j++){M[i][j]/=s||1;}}return M;}
function derive(M){const n=M.length;let pH=0,pD=0,pA=0,ov15=0,ov25=0,ov35=0,btts=0,h1=0,a1=0;const flat=[];for(let i=0;i<n;i++){for(let j=0;j<n;j++){const v=M[i][j];flat.push({i,j,v});if(i>j)pH+=v;else if(i===j)pD+=v;else pA+=v;const t=i+j;if(t>=2)ov15+=v;if(t>=3)ov25+=v;if(t>=4)ov35+=v;if(i>=1&&j>=1)btts+=v;if(i>=1)h1+=v;if(j>=1)a1+=v;}}flat.sort((a,b)=>b.v-a.v);const top=flat.slice(0,5).map(({i,j,v})=>`${i}-${j} (${v.toFixed(3)})`).join(', ');return{pHome:pH,pDraw:pD,pAway:pA,over15:ov15,over25:ov25,over35:ov35,btts,homeG1:h1,awayG1:a1,topScores:top};}
...
export default function App(){
  ...
  async function searchTeams(){
    if(!teamQ) return;
    const r=await fetch(`/api/fd?type=searchTeams&name=${encodeURIComponent(teamQ)}`);
  ...
  async function fetchTeamHistoryByName(name){
    ...
    if(!t){
      const r=await fetch(`/api/fd?type=searchTeams&name=${encodeURIComponent(name)}`); const j=await r.json();
      const arr=(j?.teams||[]).filter(tt=>tt.runningCompetitions?.some(rc=>rc.id===comp)); if(!arr.length) return [];
      t={id:arr[0].id,name:arr[0].name};
    }
    const now=new Date(); const from=new Date(now); from.setFullYear(now.getFullYear()-3);
    const r2=await fetch(`/api/fd?type=teamMatches&teamId=${t.id}&status=FINISHED&dateFrom=${from.toISOString().slice(0,10)}&dateTo=${now.toISOString().slice(0,10)}`);
  ...
  async function analyze(){
    ...
    const set=new Set(); hist.forEach(r=>{set.add(r.HomeTeam);set.add(r.AwayTeam);}); const init=()=>({m:0,gf:0,ga:0}); const map=new Map(); [...set].forEach(t=>map.set(t,init()));
    for(const r of hist){const TH=map.get(r.HomeTeam); TH.m++; TH.gf+=r.FTHG; TH.ga+=r.FTAG; const TA=map.get(r.AwayTeam); TA.m++; TA.gf+=r.FTAG; TA.ga+=r.FTHG;}
    ...
    const entries=[['1 (Local)',mk.pHome],['X (Empate)',mk.pDraw],['2 (Visitante)',mk.pAway],['Over 1.5',mk.over15],['Over 2.5',mk.over25],['Over 3.5',mk.over35],['BTTS',mk.btts],['Local ≥1',mk.homeG1],['Visitante ≥1',mk.awayG1]];
  ...
  async function loadFixtures(){
    ...
    const promises = comps.map(async c => {
      const r = await fetch(`/api/fd?type=competitionMatches&competitionId=${c.id}&status=SCHEDULED&dateFrom=${theDate}&dateTo=${theDate}`);
      const j = await r.json();
  ...
  function loadToday(){
    ...
    setTheDate(today);
    // esperar a que setState se aplique? aquí llamamos con today directamente
    (async()=>{
      const comps=COMPETITIONS;
      const promises = comps.map(async c => {
        const r = await fetch(`/api/fd?type=competitionMatches&competitionId=${c.id}&status=SCHEDULED&dateFrom=${today}&dateTo=${today}`);
        const j = await r.json();
        const matches = (j?.matches || []).map(m => ({home:m.homeTeam?.name||'-',away:m.awayTeam?.name||'-',utcDate:m.utcDate||''}));
        return { competitionId: c.id, competitionName: c.name, matches };
      });
  ...
  return (
    <div className="container">
      ...
      <div className="sub">Datos en vivo desde football-data.org. Usa <b>Partidos del día</b> o elige equipos manualmente.</div>
      ...
      <div className="grid">
          ...
          <div className="fixtures" style={{marginTop:8}}>
            {fixtures.map(section => (
              <div key={section.competitionId} className="card">
                ...
                {!section.matches.length && <div className="badge">Sin partidos programados para esta fecha.</div>}
        ...
        <div className="card">
          <div className="sub">4) Umbral de alta probabilidad</div>
        ...
        <div className="card" style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div className="sub">Descargar históricos recientes de ambos equipos y analizar</div>
